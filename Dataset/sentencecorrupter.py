...
class SentenceCorrupter:
	...
	def __init__():
		with open('wiki_history.csv', encoding = "utf8") as f0:
			...
			next(f0)
			SentenceCorrupter.corruptSentences(read, SentenceCorrupter.generateRand(row_count))

	@staticmethod
	def generateRand(row_count):
	...
	@staticmethod
	def corruptSentences(read, numlis):
			...
			for key in numlis:
				for row in read:
					if (key == 1): #if no more rows to advance
						write.writerow(SentenceCorrupter.corruption(row, tostem_tagset, lemma))
						break
					else:
						key -= 1
	...
	@staticmethod
	def corruption(row, tagset, lemma):
		...
		for word in taglisOflis:
			if word[1] in tagset: #if word is to be corrupted
